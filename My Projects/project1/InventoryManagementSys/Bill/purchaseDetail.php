...
$invoice = [];
if ($invoice_no) {
    $query = "SELECT invoice_no, invoice_date, grand_total,payment_term, seller_Name, mobileNo, address 
      FROM purchase_invoices JOIN sellers ON purchase_invoices.seller_id = sellers.seller_id 
...
<head>
  <meta charset="utf-8" />
  <title>Purchase Invoice View</title>
...
<div id="invoice">
  ...
  <div class="mb-3">
    ...
    <strong>Seller Name:</strong> <?= htmlspecialchars($invoice['seller_Name'] ?? '') ?><br>
...
<?php
    ...
    purchases.unit_price AS purchase_unit_price,
    purchases.quantity,
    purchases.total,
    purchases.discount,
    purchases.tax,
...
?>
  ...
  <!-- Product Details -->
  <table class="table table-bordered table-striped" id="PurchaseTable" border="1"> <!-- ✅ added id -->
    ...
    <tbody>
      ...
      if ($result_items) {
        while ($item = mysqli_fetch_assoc($result_items)) {
          ...
          $rate_formatted = number_format((float)$item['purchase_unit_price'],2);
