...
$sql_invoice = "SELECT * FROM sales_invoices 
...
<body>
  ...
  <div class="container-fluid">
    <form id="invoiceForm" action="saleEdited.php" method="post">
      <div class="row">
        <!-- Invoice header  -->
        <div class="col-2  ">
          ...
          <select type="text" class="customer" id="customerSelect" name="customer_name" value="">
  ...
  <div class="justify-content-center saveb">
    <button type="submit" form="invoiceForm" class="position-fixed bottom-0 end-0  mb-0 save-btn">Update Sales</button>
  </div>
...
async function loadCustomer() {
      try {
        let response = await fetch("loadCustomer.php"); // call backend
        let data = await response.json();

        let dropdown = document.getElementById("customerSelect");
        dropdown.innerHTML = `<option value=""><?=$invoice['customer_name']?></option>`;
    ...
    document.getElementById('invoiceForm').addEventListener('submit', (e) => {
      ...
      fetch("saleEdited.php", {
        ...
        .then(response => {
          ...
          window.location.href = "../Dashboard/payment.php"?invoice_id=" + data.invoice_id;
        })
        .catch(error => {
